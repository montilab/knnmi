test_that("mutual_inf_cc_1d returns expected result", {
  ...
  result <- mutual_inf_cc_1d(mutual_info_df$Xc, mutual_info_df$Zc_XcYc)
  expect_length(result, 1L)
  expect_type(result, "double")
  expect_equal(result, 0.0, tolerance = 0.00001)


  result <- mutual_inf_cc_1d(mutual_info_df$Yc, mutual_info_df$Zc_XcYc)
  expect_length(result, 1L)
  expect_type(result, "double")
  expect_equal(result, 0.2738658, tolerance = 0.00001)
})

test_that("mutual_inf_cc_1d returns expected result with seed", {
  ...
  result <- mutual_inf_cc_1d(x, y, k=3, seed=654321)
  expect_length(result, 1L)
  expect_type(result, "double")
...
test_that("mutual_inf_cc_1d issues error messages when the value of k is too large", {
  ...
  expect_error(  mutual_inf_cc_1d(x, y, k=16) )
})
