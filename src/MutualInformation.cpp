...
>>>>>>> ec06dd90deb3b00f8239bc78900dad6c88a780fa
    
  // Coerce the MI value to be in the range 0-1
  return std::min(1.0,std::max(0.0,mi)) ;
}
...
ArrayXd MutualInformation::scale(const ArrayXd &x) const {
  // Center and scale the vector x
  auto size_v = x.size() ;
  double mean_x = x.mean() ;
  auto x_p = (x - mean_x) ;
...
pair<vector<double>,vector<size_t>>  MutualInformation::calc_distances(const size_t N, const Array<double, -1, 3> &tmp_mat) const {
  // Calculate the Chebyshev distances and numbers of neighbors for the 2D array tmp_mat with 3 columns.
...
pair<vector<double>,vector<size_t>> MutualInformation::calc_distances(const size_t N, const Array<double, -1, 2> &tmp_mat) const {
  // Calculate the Chebyshev distances and numbers of neighbors for the 2D array tmp_mat  with 2 columns
